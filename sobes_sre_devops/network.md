...
## **HR-вопросы: сетевое оборудование и BGP**

### **1. Приходилось ли самому настраивать коммутаторы и маршрутизаторы?**
**Короткий ответ:**  
Базовую настройку делал: VLAN, проверка связности, таблиц маршрутизации и доступности сервисов.  
Сложные изменения обычно выполнялись совместно с сетевыми инженерами по регламенту.

### **2. Приходилось ли работать с BGP?**
**Короткий ответ:**  
Глубокой ежедневной эксплуатации было немного, но есть уверенное понимание основ: eBGP/iBGP, `AS_PATH`, `LOCAL_PREF`, фильтрация маршрутов, риски утечки префиксов.

### **Что важно не перепутать в ответе**
1. BGP — междоменная маршрутизация, не замена OSPF внутри обычной локальной сети.
2. Анонс маршрута без фильтров может привести к утечке и широкому сбою.
3. Нужно четко проговаривать, что делал лично, а что в связке с профильной сетевой командой.

### **Мини-кейсы для устной тренировки**
1. Сессия BGP не поднимается: какие проверки идут первыми (доступность, `remote-as`, фильтры, TCP/179).
2. После изменения политик часть трафика ушла по нештатному пути: как найти причину и откатить изменения.
