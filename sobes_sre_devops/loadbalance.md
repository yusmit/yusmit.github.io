...
## HAProxy для высоких нагрузок: что нужно уметь отвечать
### Ключевые идеи
- `HAProxy` часто выбирают там, где важны высокая производительность, понятное поведение под нагрузкой и контроль над L4/L7 балансировкой.
- Для high-load недостаточно сказать «ставим leastconn». Нужно понимать `maxconn`, очереди, таймауты, keepalive, health checks, поведение при деградации backend.
- Балансировщик сам может стать bottleneck, если не следить за file descriptors, CPU saturation, числом соединений и задержками на SSL termination.

### Вопрос: Что оптимизировать в HAProxy под высокой нагрузкой?
Ответ:
- Ограничения по соединениям: `maxconn`, backlog, `ulimit`, очереди на frontend/backend.
- Таймауты: `timeout connect`, `timeout client`, `timeout server`, чтобы не копить зависшие сессии.
- Health checks: не слишком редкие и не слишком агрессивные, чтобы не вызвать flapping.
- Логирование и метрики: RPS, active sessions, retries, queue time, backend response time, 5xx/4xx, отказ backend.

### Вопрос: Как понять, что проблема в HAProxy, а не в backend?
Ответ:
- Сравнить `frontend` и `backend` latency, очереди, число активных соединений и ошибки по backend.
- Проверить saturation самого узла с HAProxy: CPU, сетевые очереди, file descriptors, SSL offload.
- Сопоставить логи балансировщика с метриками приложений: если backend спокоен, а очередь растет на LB, проблема ближе к LB.

### Мини-кейс
#### После роста трафика появились очереди и 502/503
1. Проверить `maxconn`, очереди frontend/backend и лимиты ОС.
2. Проверить health checks: не выкидывают ли они backend по ложным таймаутам.
3. Проверить, не уперлись ли в SSL termination или CPU.
4. Разделить: проблема в самом LB, сети или backend-сервисах.

---

## Диагностика медленного `nginx`

### Как рассуждать правильно

Когда `nginx` "медленный", это не всегда значит, что проблема именно в нем. `nginx` может тормозить из-за:
- перегруженного backend;
- DNS;
- дисковой подсистемы;
- SSL termination;
- нехватки worker'ов или соединений;
- неудачных таймаутов;
- проблем сети между `nginx` и upstream.

Хороший ответ всегда разделяет:
1. проблема в самом `nginx`;
2. проблема за ним;
3. проблема вокруг него.

### Чеклист диагностики

#### 1. Проверить логи

- `access.log` — время ответа, коды ответа, всплески долгих запросов;
- `error.log` — таймауты, upstream errors, reset, проблемы сокетов и DNS.

Если в access log последним полем записывается request time, быстрый поиск может выглядеть так:

```bash
awk '$NF > 1' access.log
```

Это не универсально для любого log_format, но на интервью полезно проговорить идею: фильтровать медленные запросы по полю времени ответа.

#### 2. Проверить ресурсы узла

- CPU: `top`, `mpstat`;
- память: `free -h`, `vmstat`;
- диск: `iostat`, очередь I/O;
- сеть: retransmits, saturation интерфейса, ошибки.

#### 3. Проверить backend напрямую

Если `nginx` проксирует backend, надо сравнить:
- время ответа через `nginx`;
- время ответа напрямую до upstream.

Например:

```bash
curl -w "time_total: %{time_total}\n" -o /dev/null -s http://backend:8080/health
```

Если backend уже медленный напрямую, проблема не в `nginx`, а за ним.

### Что проверять в конфигурации

#### Worker'ы и соединения
...
Важно понимать:
- `worker_processes auto` обычно равен числу CPU;
- `worker_connections` задает число соединений на worker;
- если соединений больше, чем может держать процесс и ОС, начнутся отказы и очереди.
...
Идея:
- слишком короткие таймауты дают ложные ошибки;
- слишком длинные — копят зависшие соединения и забивают ресурсы.
...
Буферизация полезна, когда нужно сгладить работу с backend и клиентом, но требует памяти и должна быть осознанной.

#### Статика, gzip, кэш

- для статики полезны `expires` и cache headers;
- gzip полезен для текстового контента;
- но компрессия тоже потребляет CPU, и это надо учитывать под нагрузкой.

### Типичные узкие места

- backend отвечает медленно;
- слишком мало `worker_connections`;
- проблемный DNS lookup до upstream;
- SSL termination забирает много CPU;
- слишком много idle keepalive connections;
- медленный диск при раздаче больших файлов;
- переполненные лимиты ОС (`ulimit`, file descriptors, backlog).

### Что полезно ответить на собеседовании

Если спрашивают "как бы вы искали проблему", сильный ответ такой:
- сначала логи и метрики;
- потом разделение на `nginx` vs backend;
- потом ресурсы узла;
- потом таймауты, воркеры, соединения и SSL;
- потом уже точечная оптимизация конфига.

То есть не "сразу увеличу worker_connections", а сначала подтвержу, где именно bottleneck.
