...
*   **`ADD` (может, но не стоит):**
    ...
    **Правильная альтернатива:** Скачивайте файлы на этапе сборки с помощью `RUN` и инструментов вроде `curl` или `wget`. Это прозрачнее и позволяет тоньше управлять процессом (например, проверять checksum).
    ```dockerfile
...
## Изоляция процессов в Linux и Docker

### На чем вообще держится изоляция контейнера

Контейнер Docker — это не "маленькая виртуальная машина". Он использует ядро хоста и изолируется набором механизмов Linux.
...
- слоистая файловая система (`overlay2`/UnionFS).

### Namespaces

`Namespaces` отвечают за вопрос: что процесс видит как свою "отдельную систему".
...
- `IPC` — разделяемая память, семафоры и message queues;
- `USER` — отображение пользователей и групп;
- `CGROUP` — отдельное представление cgroup-иерархии.

Практический смысл:
- процесс внутри контейнера не обязан видеть процессы хоста;
- root внутри контейнера не обязан быть root снаружи;
- у контейнера может быть своя сеть, hostname и mount namespace.

### Cgroups

`cgroups` отвечают за вопрос: сколько ресурсов процесс или контейнер может реально использовать.
...
Именно `cgroups` позволяют сказать:
- этому контейнеру не больше 512 MiB RAM;
- не больше 1 CPU;
- ограничить burst или число дочерних процессов.

### Capabilities

Полный root в Linux — это набор привилегий. `Capabilities` позволяют выдавать их поштучно.
...
- контейнеру не всегда нужен полный root;
- можно дать только ту привилегию, которая реально требуется;
- это уменьшает blast radius при компрометации контейнера.
...
Практический смысл:
- приложение не сможет вызвать лишние syscalls;
- поверхность атаки уменьшается;
- security baseline становится жестче.

### UnionFS / `overlay2`

Контейнерный образ состоит из слоев:
- read-only слои образа;
- верхний writable layer контейнера.

Это дает:
- экономию места;
- быстрый reuse слоев;
- удобную кешируемую сборку.

Но важно помнить:
- writable layer контейнера не предназначен для постоянных данных;
- постоянные данные лучше хранить в volume/bind mount.

### Мини-вопрос: в чем разница между `namespace` и `cgroup`?

Ответ:
- `namespace` отвечает за изоляцию видимости;
- `cgroup` отвечает за лимиты и учет ресурсов.

Проще:
- namespace — "что видит процесс";
- cgroup — "сколько ресурсов ему разрешено".

---

## Слои Docker и почему `CMD` не создает слой

### Как вообще устроены слои образа

Образ Docker строится как последовательность слоев файловой системы. Слой появляется тогда, когда инструкция меняет содержимое FS образа.

Типично создают слои:
- `RUN`;
- `COPY`;
- `ADD`.

Они реально меняют файловую систему:
- устанавливают пакеты;
- копируют файлы;
- создают директории;
- распаковывают архивы.

### Что не создает полноценный файловый слой
...
в основном меняют metadata образа и конфигурацию контейнера, а не добавляют тяжелый слой данных.

#### Почему `CMD` не создает слой

`CMD` просто задает команду по умолчанию, которая будет выполнена при старте контейнера.
...
Это не изменяет файловую систему образа:
- не копирует файлы;
- не создает пакеты;
- не меняет содержимое каталогов.

Поэтому можно говорить, что `CMD` не создает полезный файловый слой и не влияет на "вес" образа как `RUN` или `COPY`.

### Почему `RUN apt update && apt install ...` пишут в одну строку

Не только ради красоты:
- меньше слоев;
- меньше мусора от промежуточных файлов;
- проще очистить package cache в том же слое;
- итоговый образ меньше и предсказуемее.
...
### Что полезно отвечать на собеседовании

- слой — это результат изменения файловой системы;
- metadata-инструкции сами по себе не делают тяжелый FS-layer;
- `CMD` задает поведение контейнера при запуске, а не содержимое образа;
- оптимизация Dockerfile — это прежде всего порядок слоев, кеширование и cleanup.

### Мини-вопросы

#### Какие инструкции точно создают слой?
Обычно `RUN`, `COPY`, `ADD`.

#### Почему `CMD` и `ENTRYPOINT` важны, даже если не создают слой?
Потому что они определяют модель запуска контейнера, обработку сигналов, удобство дебага и реальное поведение приложения.
